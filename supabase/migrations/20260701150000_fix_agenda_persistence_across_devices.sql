...
BEGIN
  ...
  IF existing_lead.id IS NOT NULL THEN
    ...
    SET
      pipeline_client_id = pc.id,
      agenda_event_id = COALESCE(existing_lead.agenda_event_id, existing_event.id),
      agenda_event_date = TO_CHAR(v_event_date, 'YYYY-MM-DD'),
      agenda_event_time = LEFT(TO_CHAR(v_event_time, 'HH24:MI:SS'), 5),
      agenda_event_title = COALESCE(NULLIF(BTRIM(existing_lead.agenda_event_title), ''), COALESCE(NULLIF(BTRIM(existing_event.title), ''), v_title)),

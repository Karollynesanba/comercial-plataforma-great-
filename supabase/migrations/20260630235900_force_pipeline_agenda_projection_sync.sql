-- Make the CRM save/delete RPCs keep agenda projections in the same transaction.
-- This protects the lead -> agenda flow even if a trigger is temporarily out of sync.
...
BEGIN
  ...
  DELETE FROM public.agenda_events WHERE pipeline_client_id = parsed_id;
  DELETE FROM public.agendamento_leads WHERE pipeline_client_id = parsed_id;

-- Prevent the pipeline trigger from creating a second agendamento_leads row
-- when the agendamento form already created the lead.

-- Clean up historical duplicates where the "manual" agendamento row and the
-- pipeline projection row point to the same person, date and time.
DELETE FROM public.agendamento_leads a
USING public.agendamento_leads b
WHERE a.id <> b.id
  AND a.pipeline_client_id IS NULL
  AND b.pipeline_client_id IS NOT NULL
  AND lower(btrim(COALESCE(a.nome, ''))) = lower(btrim(COALESCE(b.nome, '')))
  AND regexp_replace(COALESCE(a.telefone, ''), '\D', '', 'g') = regexp_replace(COALESCE(b.telefone, ''), '\D', '', 'g')
  AND COALESCE(a.data, '') = COALESCE(b.data, '')
  AND COALESCE(a.horario_especifico, '') = COALESCE(b.horario_especifico, '');
...
BEGIN
  ...
  WHERE ae.pipeline_client_id = NEW.id
     OR (
      ...
      AND (
        ...
        OR lower(btrim(COALESCE(ae.title, ''))) = lower(btrim(COALESCE('Reuniao com ' || COALESCE(NULLIF(NEW.client_name, ''), 'Lead sem nome'), '')))
      )
    )
  ORDER BY
  ...
  IF agenda_event_id IS NOT NULL THEN
    ...
    SET
      ...
      title = 'Reuniao com ' || COALESCE(NULLIF(NEW.client_name, ''), 'Lead sem nome'),

-- Additive CRM field. Existing rows remain unchanged with formulario = NULL.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'pipeline_clients_formulario_check'
      AND conrelid = 'public.pipeline_clients'::regclass
  ) THEN
    ALTER TABLE public.pipeline_clients
      ADD CONSTRAINT pipeline_clients_formulario_check
      CHECK (formulario IS NULL OR formulario IN ('S1', 'S2'));
  END IF;
END;
$$;
...
BEGIN
  ...
  ON CONFLICT (id) DO UPDATE SET
    ...
    formulario = CASE
      WHEN payload ? 'formulario' THEN EXCLUDED.formulario
      ELSE public.pipeline_clients.formulario
    END,

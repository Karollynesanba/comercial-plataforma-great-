-- Prevent agenda duplication when a pipeline update touches an existing event.
-- Reuse the agenda row for the same person and same date/time slot instead of
-- inserting a second record with the old title.

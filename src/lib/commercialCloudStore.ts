...
export async function syncPipelineAutomationsToCloud(client: PipelineClient, userId?: string | null) {
  ...
  const [{ data: existingAgendaEvent }, { data: existingAgendamentoLead }] = await Promise.all([
    supabase.from('agenda_events').select('*').eq('pipeline_client_id', client.id).maybeSingle(),
    supabase.from('agendamento_leads').select('*').eq('pipeline_client_id', client.id).maybeSingle(),
  ]);
  const agendamentoLeadPayload = {
    pipeline_client_id: client.id,
    data: leadDate,
    nome: existingAgendamentoLead?.nome || client.clientName || leadName,
    telefone: existingAgendamentoLead?.telefone || client.telefone || leadPhone,
    profession: existingAgendamentoLead?.profession || client.profession || null,
    horario: existingAgendamentoLead?.horario || timeToPeriod(client.meetingTime),
    horario_especifico: leadTime,
    tem_socio: existingAgendamentoLead?.tem_socio || commercialAnswerToDb(client.temSocio),
    tem_mkt: existingAgendamentoLead?.tem_mkt || commercialAnswerToDb(client.temMkt),
    tem_secretaria: existingAgendamentoLead?.tem_secretaria || commercialAnswerToDb(client.temSecretaria),
    salao_ou_clinica: existingAgendamentoLead?.salao_ou_clinica || client.salaoOuClinica || null,
    faturamento: existingAgendamentoLead?.faturamento || normalizePipelineFaturamento(client.faturamento),
    ...
    agenda_event_title: existingAgendamentoLead?.agenda_event_title || existingAgendaEvent?.title || defaultTitle,
    created_by_user_id: toDbUserId(userId) || toDbUserId(client.createdByUserId) || existingAgendamentoLead?.created_by_user_id || null,
  ...
  const agendaEventPayload = {
    pipeline_client_id: client.id,
    client_name: existingAgendaEvent?.client_name || client.clientName || leadName,
    client_phone: existingAgendaEvent?.client_phone || client.telefone || leadPhone,
    title: existingAgendaEvent?.title || defaultTitle,
    description: existingAgendaEvent?.description || `Lead do Pipeline - ${getCommercialLeadOrigin({ criativo: client.criativo, funil: client.funil })}`,
    ...
    color: existingAgendaEvent?.color || agendaColorForStage(leadStage),
    ...
    created_by_user_id: existingAgendaEvent?.created_by_user_id || toDbUserId(userId) || toDbUserId(client.createdByUserId) || null,
    assigned_closer_id: existingAgendaEvent?.assigned_closer_id || null,
    team_id: existingAgendaEvent?.team_id || null,
